...
function spawnPowerUp() {
    ...
    const types = ['triple', 'spread5'];
    const type = types[Math.floor(Math.random() * types.length)];
    powerUps.push(new PowerUp(x, y, type));

    setTimeout(() => {
        powerUps.shift(); // Remove power-up after some time if not collected
    }, 8000);
}
...
function activatePowerUp(type) {
    ...
    powerUpTimer = setTimeout(() => {
        activePowerUp = null; // Revert to normal projectiles after 10 seconds
    }, 10000);
}
...
// Function to make enemies shoot every 2 seconds
...
function animate() {
    ...
    // Add power-up collision detection in animate function
...
// Modify shootProjectile function
function shootProjectile(clientX, clientY) {
    ...
    if (activePowerUp === 'triple') {
        ...
        projectiles.push(new Projectile(player.x, player.y, 5, 'white', {
            x: Math.cos(angle + 0.2) * 5,
            y: Math.sin(angle + 0.2) * 5,
        }));
        projectiles.push(new Projectile(player.x, player.y, 5, 'white', {
            x: Math.cos(angle - 0.2) * 5,
            y: Math.sin(angle - 0.2) * 5,
        }));
    } else if (activePowerUp === 'spread5') {
        projectiles.push(new Projectile(player.x, player.y, 5, 'white', velocity));
        projectiles.push(new Projectile(player.x, player.y, 5, 'white', {
            x: Math.cos(angle + 0.3) * 5,
            y: Math.sin(angle + 0.3) * 5,
        }));
        projectiles.push(new Projectile(player.x, player.y, 5, 'white', {
            x: Math.cos(angle - 0.3) * 5,
            y: Math.sin(angle - 0.3) * 5,
        ...
        projectiles.push(new Projectile(player.x, player.y, 5, 'white', {
            x: Math.cos(angle + 0.15) * 5,
            y: Math.sin(angle + 0.15) * 5,
        }));
        projectiles.push(new Projectile(player.x, player.y, 5, 'white', {
            x: Math.cos(angle - 0.15) * 5,
            y: Math.sin(angle - 0.15) * 5,
